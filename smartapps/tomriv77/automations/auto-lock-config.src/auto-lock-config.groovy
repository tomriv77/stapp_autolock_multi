...
preferences {
	page name: "mainPage", install: false, uninstall: true, nextPage: "namePage"
...
def namePage() {
    if (!overrideLabel) {
        ...
        def l = contactSensor.displayName
    ...
    dynamicPage(name: "namePage") {
        if (overrideLabel) {
            section("Location name") {
                label title: "Enter custom name", defaultValue: contactSensor.displayName, required: false
            }
        } else {
            section("Location name") {
                paragraph contactSensor.displayName
            }
        }
        section {
            input "overrideLabel", "bool", title: "Edit Location name", defaultValue: "false", required: "false", submitOnChange: true
        }
    }
}
...
def initialize() {
    ...
    if (!overrideLabel) {
        app.updateLabel(contactSensor.displayName)
    }
}
...
{
	def msg = "${app.label} has been open for ${elapsedInMin} minute(s)."
	log.info msg
